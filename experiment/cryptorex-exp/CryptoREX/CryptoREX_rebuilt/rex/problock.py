...
class function:
    def __init__(self):
        ...
        self.addr = list()	# ?
        self.arg = list()
        self.farg = list()	# types of taint sources
...
class traceinfo:
    ...
    # Add transfer target (maybe a taint sink)
    ...
    def keyupdate(self, v, vtype, nv, nvtype):
        ...
        self.vl[index] = int(nv)
        self.vtypel[index] = nvtype
        return True

    # Set up a transfer edge
    ...
    def listallpp(self, filename, call_stack, path, dt):
        msg = ""
        for i in range(0, len(self.vl)):
            ...
            print(msg)
            # Report
            function_name = self.functionname + f' ({hex(self.faddrl[i])})'
            report.vulnerabilities.append(ViolationDetail(
                filename=filename,
                function_name=function_name,
                call_stack=call_stack,
                path=path,
                data=str(self.fresultl[i]),
                rule=self.rulel[i]
            ))
            
            dt[str(self.functionname)+str(self.faddrl[i])+str(self.metal[i]) +
                   str(self.calleaddr[i])+str(self.fresultl[i])] = (msg, self.rulel[i], self.ruledesl[i])
...
def func2traceinfo(function, beginaddr):
    ...
    indexi = 0
    # traverse all tainted arguments
...
class functioncontainer:
    ...
    def addfunction(self,function):
        if function.getfunctionname() in self.functionsdic or any(s in function.getfunctionaddr() for s in list(self.functionsdic.keys())):
            return
    ...
    def __str__(self) -> str:
        return '''{
            functions: %s
            functionsdic: %s
        }''' % (self.functions, self.functionsdic)
...
class problock:
    def __init__(self,irsb=None,lcom = None):
        ...
        if not irsb == None:
            ...
            if self.irsb.arch.vex_arch == "VexArchARM":
                ...
                self.argunfixed.append(self.irsb.arch.get_register_offset("r2"))
                self.argunfixed.append(self.irsb.arch.get_register_offset("r3"))
            self.maxp = 2 ** (self.arch.bits - 1) - 1	# Max postive number
            self.minn = -2 ** (self.arch.bits - 1)	# Min negative number
            self.wordlen = self.irsb.arch.bits // 8
    
    def __repr__(self) -> str:
        return f'problock: {hex(self.beginaddr)}'
    ...
    def getnextblockaddr(self,spbpoffset=None):
        ...
        vmax = 0
        voff = 0
        # Example: {'60 (1,)': 0, '0 (4,)': 0, '29 (4,)': -32, '30 (4,)': -4, '32 (4,)': -8, '34 (4,)': -12, '36 (4,)': -16, '38 (4,)': -20, '40 (4,)': -24, '42 (4,)': -28}
        ...
        for sitem in self.irsb.statements:
            if sitem.tag == "Ist_IMark":
                ...
                # Before calling
                # [!!!] Problematic
                # Only arm&mips save the return address to link register before calling another function
            ...
            if sitem.tag == "Ist_WrTmp":
                ...
                if eitem.tag == "Iex_Unop":
                    ...
                    if eitem.op == "Iop_Add32":
                        arg1 = eitem.args[0]
                        arg2 = eitem.args[1]
                        # [???] Could arg1 and arg2 be both "Iex_RdTmp" ?
        ...
        for i in range(0,len(tmpresult)):
            ...
            if sitem.tag == "Ist_Exit":
                # if (t11) { PUT(offset=136) = 0x40553c; Ijk_Boring }
    ...
    def translatetmp2function(self,nowaddr,pastaddr):
        if nowaddr in self.lcom and (self.lcom[nowaddr][0] == ";"):
            return self.lcom[nowaddr][1:]
        elif pastaddr in self.lcom and (self.lcom[pastaddr][0] == ";"):
