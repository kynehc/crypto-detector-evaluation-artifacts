 ...
 */
public abstract class CustomFlowAnalysis<N, A> extends AbstractFlowAnalysis<N, A> {
  public enum Flow {
    IN {
      @Override
  ...
  enum InteractionFlowHandler {
    ...
    FORWARD {
      @Override
      public <A, N> void handleFlowIn(CustomFlowAnalysis<N, A> a, N s) {
        beforeEvent(stop(s), a, s);
      }

      @Override
      public <A, N> void handleFlowOut(CustomFlowAnalysis<N, A> a, N s) {
    ...
    <A, N> void beforeEvent(InteractionHandler i, CustomFlowAnalysis<N, A> a, N s) {
    ...
    <A, N> void afterEvent(InteractionHandler i, CustomFlowAnalysis<N, A> a, N s) {
  ...
  public CustomFlowAnalysis(DirectedGraph<N> graph) {
    super(graph);
  ...
  final int doAnalysis(
    ...
    for (int numComputations = 0; ; numComputations++) {
      ...
      // my fix: fork UnitContainer for isolated useBoxes in each branch
      if (e.out.length > 1) {
        for (int i = 0; i < e.out.length; i++) {
          ValueArraySparseSet copyFlowSet = new ValueArraySparseSet();
          for (Object uc : ((FlowSet) e.outFlow).toList()) {
            try {
              copyFlowSet.add(((UnitContainer) uc).clone());
            } catch (CloneNotSupportedException ex) {
              ex.printStackTrace();
            }
          }
          e.out[i].inFlow = (A) copyFlowSet;
        }
      }
